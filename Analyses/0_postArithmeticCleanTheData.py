...
for i in range(1, NumberOfParticipants + 1):
    df = pd.read_csv(f"post-tests/arithmetic_{i:02}.csv") #format of your participant code (leading 0 if single digit here)
...
temp_tra.to_csv("post_arithmetic_clean_training.csv", index = False)
temp_exp.to_csv("post_arithmetic_clean_experiment.csv", index = False)